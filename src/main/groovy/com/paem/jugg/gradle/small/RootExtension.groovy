...
public class RootExtension extends BaseExtension {
  ...
  private static final VersionNumber REQUIRED_AAR_REVISION = VersionNumber.parse(
      REQUIRED_AAR_VERSION)
  ...
  protected String mP
  // the executing gradle project name
  protected String mT
  // the executing gradle task name
  ...
  public String getAarVersion() {
    if (aarVersion == null) {
      ...
      if (PLUGIN_REVISION < VersionNumber.parse('1.1.0-alpha2')) {
        throw new RuntimeException('Please specify Small aar version in your root build.gradle:\n' +
  ...
  public File getBundleOutput(String bundleId) {
    ...
    if (buildToAssets) {
      ...
      def arch = System.properties['bundle.arch']
      // Get from command line (-Dbundle.arch=xx)
      if (arch == null) {
      ...
      def so = "lib${bundleId.replaceAll('\\.', '_')}.apk"
