package com.paem.jugg.gradle.jar
...
class BuildJarPlugin implements Plugin<Project> {
  ...
  public void apply(Project project) {
    ...
    if (project.getPlugins().hasPlugin(AppPlugin)) {
      println "${AppConstant.TAG} create extension:" + EXTENSION_NAME
        ...
        if (variant.name.capitalize() == "Debug") {
          ...
          if (dexTask != null) {
            ...
            Closure buildJarClosure = {
              ...
              if ((includeClass == null || includeClass.size() <= 0) && (includePackage == null ||
                  includePackage.size() <=
                  0)) {
            ...
            project.task(buildJarBeforeDex) << {
              ...
              inputFiles.each { inputFile ->
                ...
                if (path.endsWith(SdkConstants.DOT_JAR) && !BuildJarUtils.isExcludedJar(path,
                    excludeJar)) {
            ...
            //混淆后输出的jar包
            buildProguardJar.outjars(
                jarExtension.outputFileDir + "/" + jarExtension.outputProguardFileName)

            //libraryjars表示引用到的jar包不被混淆
            // ANDROID PLATFORM
            buildProguardJar.libraryjars(project.android.getSdkDirectory().toString() +
                "/platforms/" +
            ...
            if (jarExtension.needDefaultProguard) {
              buildProguardJar.configuration(
                  project.android.getDefaultProguardFile('proguard-android.txt'))
            }
            //applymapping
